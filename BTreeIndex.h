...
class BTreeIndex {
 public:
  ...
  /**
   * When new node is inserted into the root and root need to be split
   * @param key[IN] the key to be inserted 
   * @param curRootPid[IN] current root page id
   * @param newRootPid[IN] new root page id
   */
  
  RC createNewRoot(int key, PageId curRootPid, PageId newRootPid);

  /*
  * @param key[IN] the key to be inserted
  * @param rid[IN] RecordId of the recorded to be inserted
  * @param pid[IN] page id for page
  * @param lev[IN] total levels of the tree
  * @param ofKey[OUT] id of the overflow page
  * @param ofPage[OUT] key of the overflow
  * @return error code. 0 if no error
  */
  RC RecInsert(int& key, const RecordId& rid, PageId pid, int lev, int& ofKey, PageId& ofPage);

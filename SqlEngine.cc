...
RC SqlEngine::load(const string& table, const string& loadfile, bool index)
{
  ...
  if (rf.open(table+".tbl",'w')) {
    cout << "Error occurred when opening the record file." << endl;
    return 1;
  }
  ...
  if (!lf.is_open()) {
    cout << "Error occurred when opening the loadfile." << endl;
    return 1;
  }
  ...
  while(lf.good()) {
    ...
    if (rf.append(key, value, temp)) {
      cout << "Line could not be added to RecordFile" << endl;
      continue;
    }
  }
  rf.close();
